...
class MyButton:
    ...
    def __init__(self, text, position_center, bg_color, text_color, font,blink = False, border_stroke = 2, border_color = (0,0,0), timer_blink_color = (0,255,0), text_blink_color = (0,0,0)):
        self.text = text
        ...
        self.__blink = blink
        self.__original_color = bg_color
    ...
    def draw(self, screen):
        if self.__blink and self.__timer_blink < self.__timer_blink_max:
            ...
            self.__timer_blink = 0
            if self.bg_color == self.__timer_blink_color:
                self.set_original_color()
            else:
                self.set_blink_color()
        else:
            self.bg_color = self.__original_color
        pygame.draw.rect(screen, self.bg_color, pygame.Rect(self.position[0] - self.width // 2, self.position[1] - self.height // 2, self.width, self.height))
    ...
    def set_blink_color(self):
        self.bg_color = self.__timer_blink_color
